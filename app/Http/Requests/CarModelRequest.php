...
class CarModelRequest extends FormRequest {
    ...
    public function authorize(): bool {
        if (request()->routeIs('car_models.store')) {
            return auth()->user()->can('car_models.create');
        }

        if (request()->routeIs('car_models.update')) {
    ...
    public function rules(): array {
        ...
        return [
            ...
            'name_ar' => [
                ...
                "string",
                "max:255",
                Rule::unique('car_models', 'name_ar')->ignore(request()->route('id') ?? null)
            ],
            'name_en' => [
                "somtimes",
                "string",
                "max:255",
                Rule::unique('car_models', 'name_en')->ignore(request()->route('id') ?? null)
            ],
